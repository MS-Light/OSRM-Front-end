...
function makeIcon(i, n) {
  ...
  var markerList = ['images/marker-start-icon-2x.png', 'images/marker-end-icon-2x.png', 'images/marker-icon-2x.png'];
  if (i === 0) {
    return L.icon({
...
function addWaypoint(waypoint) {
  ...
  // if (!length) {
  //   lrmControl.spliceWaypoints(0, 1, waypoint);//Here's what we want to change.
  // } else {

    var url = "src/geolocation.json"
    ...
    request.onload = function () {
        if (request.status == 200) {
            ...
            var startpoint = json[0];
            //waypoint = (json[0].lat, json[0].lng);
            lrmControl.spliceWaypoints(0, 1, startpoint);
            console.log(json[0].lat);
            console.log(json[0].lng);
        }
    }
...
setInterval(function(){updateGeolocation()},5000);

// var fs = require('fs');
// function writeJson(){
//   fs.readFile('/src/geolocation.json',function(err,data){
//       if(err){
//           return console.error(err);
//       }
//       var geoloc = data.toString();
//       geoloc = JSON.parse(geoloc);
//       var params = geoloc.data[0];
//       params.lat -= 0.00001;
//       geoloc.data[0] = params;
//       console.log(geoloc.data);
//       var str = JSON.stringify(geoloc);
//       fs.writeFile('/src/geolocation.json',str,function(err){
//           if(err){
//               console.error(err);
//           }
...
// setInterval(function(){writeJson()},10);
